...
class ActionExecutive(Node):
    def __init__(self):
        ...
        try:
            raw_map = np.load('/home/yug/inflated_binary_grid.npy')
    ...
    # 🟢 THE 2D MAP GENERATOR
    def publish_map(self):
        msg = OccupancyGrid()
        msg.header.frame_id = 'map'
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.info.resolution = 0.1  # 10 pixels = 1 meter
        msg.info.width = 1024
        msg.info.height = 1024
        
        # Center the map perfectly under the robot
        msg.info.origin.position.x = -51.2
        msg.info.origin.position.y = -51.2
        
        # Format the numpy array to standard ROS 2 map data (0 = free, 100 = wall)
    ...
    def odom_callback(self, msg):
        ...
        hab_yaw = math.atan2(msg.pose.pose.orientation.z, msg.pose.pose.orientation.w) * 2
        yaw = math.atan2(-math.cos(hab_yaw), -math.sin(hab_yaw))
        ...
        if path and len(path) > 1:
            ...
            if abs(angle_diff) < 0.4:
                ...
                self.current_action = "turn_right" if angle_diff > 0 else "turn_left"
        else:
            self.current_action = "turn_left"

    def timer_callback(self):
        # 🟢 NEW SAFETY GATE: Only run if the goal is actually defined
        if self.planner.goal_cell is None:
            # Optionally log that we are waiting
